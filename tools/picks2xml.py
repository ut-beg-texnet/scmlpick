...
def read_params(params_file='params.inp'):
    try:
    ...
    for line in lines:
        if line[0] == '#' or line.strip('\n').strip() == '':
            ...
            try:
                ...
                # Try converting the value to various data types
                try:
                    value = int(value)
                except ValueError:
                    try:
                ...
                print(f"Error: Invalid line format in file '{params_file}': {line}")
    
    # print(f"Parameters file ({params_file}) parsed. Total of {len(params_dict)} parameters.")
    return params_dict
